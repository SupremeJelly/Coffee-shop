        ...
        public void displayData()
        {
            ...
            foreach (var doc in documents)
            {
                dataGridView1.Rows.Add(
                    i,
                    doc.GetValue("MaHang", "").ToString(),
                    doc.GetValue("TenHang", "").ToString(),
                    doc.GetValue("SoLuong", 0).ToInt32(),
                    doc.GetValue("DonViTinh", "").ToString(),
                    doc.GetValue("Gia", 0).ToDouble(),
                    doc.GetValue("TrangThai", "").ToString()
                );
                i++;
            }
        }
